/**
 * Annotation geometry: dimensions, notes with leader lines, and guides.
 * It lives outside the edge graph — none of it becomes a face or shows up in a
 * mesh export. Labels leave here as 3D positions; the Viewport is what draws
 * them in the DOM, so the text stays crisp and always faces the camera.
 */
...
export type AnnotationKind = 'dimension' | 'note' | 'guide';
...
/** Any vector perpendicular to `d`, for a dimension created with no offset. */
function anyPerp(d: Vec3): Vec3 {
  const helper: Vec3 = Math.abs(d[2]) < 0.9 ? [0, 0, 1] : [1, 0, 0];
  return norm3(cross3(helper, d));
}

/** Guide reach and tick size, proportional to the model. */
...
export function buildAnnotations(
  ...
  for (const dim of model.dimensions.values()) {
    const key = refKey('dimension', dim.id);
  ...
  for (const note of model.notes.values()) {
    ...
    out.labels.push({ key, kind: 'note', id: note.id, text: note.text, pos: tip, selected: isSel });
  }
  ...
  for (const guide of model.guides.values()) {
    const isSel = selected.has(refKey('guide', guide.id));
    if (guide.dir) {
